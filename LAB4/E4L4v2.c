...
int disp_rip(int *sol, int k, int n,int pos,int z,int s,int r,int t,int *mark,int max_coll, int **sol_max,int *length_max,int rip);
 ...
 int generaCollane(int z,int r, int t, int s){
     ...
     int *best_solution=malloc((z+t+r+s)*sizeof(int));
       ...
       // for(int i=0;i<length;i++){
        printf("calcolo lunghezza %d\n",i);
            max_curr=disp_rip(sol,i,z+r+t+s,0,z,s,r,t,mark,max_val,&sol_max,&length_curr,rip); 
    ...
    int prec;
     ...
     for(i=0;i<diff_pietre ;i++){
         if(pos!=0) {
            prec=sol[pos-1];
            int flag;
            if(sol[7]==smeraldo && i==smeraldo && pos==8) {
                flag=0;}
            if(prec==zaffiro && ( ( i==zaffiro &&mark[i]<z  ) || (i==rubino && mark[i]<r))  ) {
                ...
                sol[pos]=i;
                mark[i]++;
                max_coll=disp_rip(sol,k,n,pos+1,z,s,r,t,mark,max_coll,sol_max,length_max,rip);
                mark[i]--;
                if (prec==i) rip--;
                }
            }
            else if(prec==smeraldo && ((i==smeraldo && mark[i]<s) || (i==topazio && mark[i]<t))  ) {
                if (prec==i&& rip+1<=max_rip)rip++;
                else rip=1;
                if(rip<=max_rip){
                sol[pos]=i;
                mark[i]++;
