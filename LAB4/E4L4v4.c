...
int max_rip;
 ...
 int piazzaPietre(int *sol, int k, int n,int pos,int *mark,int max_coll, int **sol_max,int *length_max,int *max_mark,int i);
int disp_rip(int *sol, int k, int n,int pos,int *mark,int max_coll, int **sol_max,int *length_max,int *max_mark);
 ...
 int main(){
    ...
    for(int i=0;i<length_file;i++){
        printf("TEST #%d\n",i+1);
        fscanf(fp,"%d %d %d %d %d %d %d %d %d",&mark[0],&mark[1],&mark[2],&mark[3],&val_pietre[0],&val_pietre[1],&val_pietre[2],&val_pietre[3],&max_rip);
 ...
 int generaCollane(int*max_mark){
     ...
     int *best_solution=malloc(length*sizeof(int));
        for(int i=length;i>0;i--){
            max_curr=disp_rip(sol,i,length,0,mark,max_val,&sol_max,&length_curr,max_mark); 
     ...
     for(i=0;i<diff_pietre ;i++){
         ...
         if (flag){
             if(n_rip(sol,pos,i)<=max_rip){
                max_coll=piazzaPietre(sol,k,n,pos,mark,max_coll,sol_max,length_max,max_mark,i);
                }
         }
         }
         else {
...
int piazzaPietre(int *sol, int k, int n,int pos,int *mark,int max_coll, int **sol_max,int *length_max,int *max_mark,int i){
    sol[pos]=i;
    mark[i]++;
    max_coll=disp_rip(sol,k,n,pos+1,mark,max_coll,sol_max,length_max,max_mark);
    mark[i]--;
    return max_coll;
}