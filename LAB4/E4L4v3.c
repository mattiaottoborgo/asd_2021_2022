...
int max_rip;
 ...
 void piazzaPietra(int *sol, int k, int n,int pos,int z,int s,int r,int t,int *mark,int *max_coll, int **sol_max,int *length_max,int i);
         ...
         if(pos!=0) {
            ...
            if(prec==zaffiro && ( ( i==zaffiro &&mark[i]<z  ) || (i==rubino && mark[i]<r))  ) {
                if(n_rip(sol,pos,i)<=max_rip){
                    piazzaPietra(sol,k,n,pos+1,z,s,r,t,mark,max_coll,sol_max,length_max,i);
                }
            }
