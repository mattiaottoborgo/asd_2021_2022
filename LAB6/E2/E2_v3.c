...
sol calcColl(sol ****M,int *pietre,int pietra_id,pietra*listaSucc,int n_pietre,sol prec);
...
sol solCopy(sol s);
 ...
 int generaCollane(int*pietre,pietra *listaSucc){
    ...
    for(i=0;i<diff_pietre;i++){
        ...
        sol b=calcColl(M[i],pietre,i,listaSucc,n_pietre,prec);
        pietre[i]++;
        if(b.l>max.l)max=b;
    }
    printSolution(max);
...
sol calcColl(sol ****M,int *pietre,int pietra_id,pietra*listaSucc,int n_pietre,sol prec){
    ...
    M[pietre[zaffiro]][pietre[rubino]][pietre[topazio]][pietre[smeraldo]].vett[k]=pietra_id;
    ...
    //cambio la migliore solo se la supera

    //calcolo valore mettendo prima pietra possibile
    pietre[succ1]--;
    sol r1 =calcColl(M,pietre,succ1,listaSucc,n_pietre,tmp);
    pietre[succ1]++;

    //calcolo valore mettendo seconda pietra possibile
    pietre[succ2]--;
    sol r2= calcColl(M,pietre,succ2,listaSucc,n_pietre,tmp);
    pietre[succ2]++;
