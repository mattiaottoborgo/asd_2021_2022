...
void test_ord(corsa *arr_tratte,int n_tratte){
    ...
    corsa *arr_tratte_p_data[MAX_RIGHE];
    corsa *arr_tratte_p_cod[MAX_RIGHE];
    corsa *arr_tratte_p_staz_p[MAX_RIGHE];
    corsa *arr_tratte_p_staz_a[MAX_RIGHE];
        
    for(int i=0;i<n_tratte;i++){ //assegnazione ai vettori di puntatori a struct
            arr_tratte_p_data[i]=arr_tratte_p_cod[i]=arr_tratte_p_staz_p[i]=arr_tratte_p_staz_a[i]=arr_tratte+i;
    }
