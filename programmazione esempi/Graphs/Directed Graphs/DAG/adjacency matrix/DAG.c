...
//funzione che restituisce indice di una stringa contenuta nel grafo
int DAGgetIndex(Dag D, char *label) {
  int id;
  id = STsearch(D->tab, label);
  if (id == -1) { //ha senso nel momento in cui si vogliono aggiungere nuovi vertici
    printf("new vertex!\n");
    id = STsize(D->tab);
    STinsert(D->tab, label);
  }
  return id;
}
...
//funzione che crea lista di archi a partire da matrice delle adiacenze
